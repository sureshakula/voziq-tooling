...
def query_catchup_data(conn: sqlite3.Connection, branch_name: str, since_time: str) -> Dict[str, Any]:
    """
    ...
    json_handler.log_operation(
        "catchup_query", {"branch": branch_name, "new_posts": new_posts_count, "new_comments": new_comments_count}
    )
    return {
...
def get_last_active(conn: sqlite3.Connection, branch_name: str) -> Optional[str]:
    ...
    """
    row = conn.execute("SELECT last_active FROM agents WHERE branch_name = ?", (branch_name,)).fetchone()
...
def update_last_active(conn: sqlite3.Connection, branch_name: str) -> str:
    ...
    conn.execute("UPDATE agents SET last_active = ? WHERE branch_name = ?", (now_iso, branch_name))
    conn.commit()
    return now_iso
...
def _count_new_posts(conn: sqlite3.Connection, since_time: str) -> int:
    ...
    row = conn.execute("SELECT COUNT(*) as cnt FROM posts WHERE created_at > ?", (since_time,)).fetchone()
    return row["cnt"] if row else 0


def _count_new_comments(conn: sqlite3.Connection, since_time: str) -> int:
    """Count new comments since the given time."""
    row = conn.execute("SELECT COUNT(*) as cnt FROM comments WHERE created_at > ?", (since_time,)).fetchone()
    return row["cnt"] if row else 0


def _get_unread_mentions(conn: sqlite3.Connection, branch_name: str) -> List[Dict[str, Any]]:
...
def _get_replies(conn: sqlite3.Connection, branch_name: str, since_time: str) -> List[Dict[str, Any]]:
...
def _get_trending_post(conn: sqlite3.Connection) -> Optional[Dict[str, Any]]:
    ...
    trending_since = (datetime.now(timezone.utc) - timedelta(hours=24)).strftime("%Y-%m-%dT%H:%M:%SZ")
    row = conn.execute(
        "SELECT id, title, vote_score, room_name FROM posts WHERE created_at > ? ORDER BY vote_score DESC LIMIT 1",
        (trending_since,),
    ).fetchone()
    return dict(row) if row else None


def _get_karma_change(conn: sqlite3.Connection, branch_name: str, since_time: str) -> int:
