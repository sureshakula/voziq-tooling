...
def _set_notification_level(args: List[str], level: str) -> dict:
    ...
    try:
        ...
        if target_type == "room":
            row = conn.execute("SELECT name FROM rooms WHERE name = ?", (target_id.lower(),)).fetchone()
            if not row:
                close_db(conn)
            ...
            row = conn.execute("SELECT id FROM posts WHERE id = ?", (post_id_int,)).fetchone()
            if not row:
                close_db(conn)
        ...
        if success:
            json_handler.log_operation(
                "notification_set", {"agent": agent_name, "level": level, "target_type": target_type}
            )
            return {
                "success": True,
                "level": level,
