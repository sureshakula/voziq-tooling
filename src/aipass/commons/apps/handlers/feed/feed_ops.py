...
def display_feed(args: List[str]) -> dict:
    ...
    try:
        ...
        # Count
        total = conn.execute(f"SELECT COUNT(*) FROM posts p {where_clause}", params).fetchone()[0]
        ...
        close_db(conn)
        logger.info("[commons.feed] feed query sort=%s total=%d room=%s", sort, total, room_name)
