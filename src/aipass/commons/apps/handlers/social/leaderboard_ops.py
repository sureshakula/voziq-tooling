...
def _query_artifacts(conn: sqlite3.Connection) -> List[Dict[str, Any]]:
    ...
    rows = conn.execute(
        "SELECT owner, COUNT(*) as cnt FROM artifacts GROUP BY owner ORDER BY cnt DESC LIMIT 10"
    ).fetchall()
...
def _query_posts(conn: sqlite3.Connection) -> List[Dict[str, Any]]:
    ...
    rows = conn.execute(
        "SELECT branch_name, post_count FROM agents WHERE post_count > 0 ORDER BY post_count DESC LIMIT 10"
    ).fetchall()
...
def _query_karma(conn: sqlite3.Connection) -> List[Dict[str, Any]]:
    ...
    rows = conn.execute("SELECT branch_name, karma FROM agents WHERE karma > 0 ORDER BY karma DESC LIMIT 10").fetchall()
