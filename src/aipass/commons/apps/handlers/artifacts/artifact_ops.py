...
def _find_registry_path() -> str:
    """Walk up from __file__ to find AIPASS_REGISTRY.json in the project root."""
...
def craft_artifact(args: List[str]) -> dict:
    ...
    try:
        ...
        close_db(conn)
        logger.info(
            "[commons.artifacts] crafted %r rarity=%s creator=%s",
            name,
            rarity,
            creator,
        )
        json_handler.log_operation(
            "craft_artifact",
            {"artifact_id": artifact_id, "name": name, "rarity": rarity, "creator": creator},
        )
...
def collab_artifact(args: List[str]) -> dict:
    ...
    if len(args) < 3:
        return {
            "success": False,
            "error": 'Usage: commons collab "name" "description" @signer1 @signer2 [--rarity rare]',
        }
...
def sign_artifact(args: List[str]) -> dict:
    ...
    try:
        ...
        if signer not in required_signers:
            ...
            return {
                "success": False,
                "error": f"You are not a required signer. Required: {', '.join(required_signers)}",
            }
