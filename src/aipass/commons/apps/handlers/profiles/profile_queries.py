...
def update_bio(conn: sqlite3.Connection, branch_name: str, bio: str) -> bool:
    ...
    """
    cursor = conn.execute("UPDATE agents SET bio = ? WHERE branch_name = ?", (bio, branch_name))
    conn.commit()
...
def update_status(conn: sqlite3.Connection, branch_name: str, status: str) -> bool:
    ...
    Returns:
        True if updated, False if agent not found
    """
    cursor = conn.execute("UPDATE agents SET status = ? WHERE branch_name = ?", (status, branch_name))
    conn.commit()
    return cursor.rowcount > 0
...
def update_role(conn: sqlite3.Connection, branch_name: str, role: str) -> bool:
    ...
    Returns:
        True if updated, False if agent not found
    """
    cursor = conn.execute("UPDATE agents SET role = ? WHERE branch_name = ?", (role, branch_name))
    conn.commit()
    return cursor.rowcount > 0
...
def get_activity_stats(conn: sqlite3.Connection, branch_name: str) -> Optional[Dict[str, Any]]:
    ...
    """
    row = conn.execute(
        "SELECT post_count, comment_count, karma, joined_at, last_active FROM agents WHERE branch_name = ?",
        (branch_name,),
    ).fetchone()
...
def increment_post_count(conn: sqlite3.Connection, branch_name: str) -> None:
    ...
    Args:
        ...
        branch_name: The branch to update
    """
    conn.execute("UPDATE agents SET post_count = post_count + 1 WHERE branch_name = ?", (branch_name,))
...
def increment_comment_count(conn: sqlite3.Connection, branch_name: str) -> None:
    ...
    Args:
        ...
        branch_name: The branch to update
    """
    conn.execute("UPDATE agents SET comment_count = comment_count + 1 WHERE branch_name = ?", (branch_name,))
...
def get_all_agents_brief(conn: sqlite3.Connection) -> List[Dict[str, Any]]:
    ...
    """
    rows = conn.execute("SELECT branch_name, status, role, karma FROM agents ORDER BY karma DESC").fetchall()
