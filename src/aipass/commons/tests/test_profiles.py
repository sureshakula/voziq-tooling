...
def test_format_time_ago_hours() -> None:
    ...
    five_h_ago = (datetime.now(timezone.utc) - timedelta(hours=5)).strftime("%Y-%m-%dT%H:%M:%SZ")
...
def test_format_time_ago_days() -> None:
    ...
    three_d_ago = (datetime.now(timezone.utc) - timedelta(days=3)).strftime("%Y-%m-%dT%H:%M:%SZ")
