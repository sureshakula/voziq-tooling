...
def test_set_mood(mock_logger: object, initialized_db: sqlite3.Connection) -> None:
    ...
    row = initialized_db.execute("SELECT mood FROM rooms WHERE name = ?", ("general",)).fetchone()
...
def test_set_flavor(mock_logger: object, initialized_db: sqlite3.Connection) -> None:
    ...
    row = initialized_db.execute("SELECT flavor_text FROM rooms WHERE name = ?", ("general",)).fetchone()
...
def test_set_entrance(mock_logger: object, initialized_db: sqlite3.Connection) -> None:
    ...
    row = initialized_db.execute("SELECT entrance_message FROM rooms WHERE name = ?", ("general",)).fetchone()
