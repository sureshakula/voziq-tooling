...
(Claude Code, Codex, Gemini), branch detection, model tag helpers,
...
# =============================================
# RESOLVE GEMINI SLUG TESTS
# =============================================


class TestResolveGeminiSlug:
    """Tests for _resolve_gemini_slug."""

    def test_resolves_slug_to_branch(self):
        """Should resolve slug to branch via projects.json."""
        mod, _, _, _, _ = _import_filesystem_handler()
        handler = _make_handler(mod)
        projects_data = json.dumps({"projects": {"/home/user/Projects/AIPass/src/aipass/prax": "my-slug"}})
        with patch.object(Path, "exists", return_value=True):
            with patch.object(Path, "read_text", return_value=projects_data):
                result = handler._resolve_gemini_slug("my-slug")
        assert result == "PRAX"

    def test_no_projects_file(self):
        """Should return None when projects.json does not exist."""
        mod, _, _, _, _ = _import_filesystem_handler()
        handler = _make_handler(mod)
    ...
    def test_os_error_reading_projects(self):
        """Should return None on OSError reading projects.json."""
        mod, _, _, _, _ = _import_filesystem_handler()
        handler = _make_handler(mod)
        with patch.object(Path, "exists", return_value=True):
    ...
    def test_json_error_reading_projects(self):
    ...
    def test_slug_not_found(self):
        """Should return None when slug is not in projects.json."""
        mod, _, _, _, _ = _import_filesystem_handler()
        handler = _make_handler(mod)
        projects_data = json.dumps({"projects": {"/some/path": "other-slug"}})
        with patch.object(Path, "exists", return_value=True):
            with patch.object(Path, "read_text", return_value=projects_data):
                result = handler._resolve_gemini_slug("my-slug")
        assert result is None
...
# =============================================
# GET GEMINI BRANCH TESTS
# =============================================


class TestGetGeminiBranch:
    """Tests for _get_gemini_branch."""

    def test_cached_branch(self):
        """Should return cached branch if present."""
        mod, _, _, _, _ = _import_filesystem_handler()
        handler = _make_handler(mod)
        handler._session_branches["gkey"] = "CACHED"
        result = handler._get_gemini_branch("/fake/file.json", "gkey")
        assert result == "CACHED"

    def test_extracts_slug_from_path(self):
        """Should extract slug from path and resolve it."""
        mod, _, _, _, _ = _import_filesystem_handler()
        handler = _make_handler(mod)
        file_path = "/home/user/.gemini/tmp/my-project/chats/session-1.json"
        with patch.object(handler, "_resolve_gemini_slug", return_value="PRAX"):
            result = handler._get_gemini_branch(file_path, "gkey2")
        assert result == "PRAX"
        assert handler._session_branches["gkey2"] == "PRAX"

    def test_no_slug_returns_gemini(self):
        """Should return GEMINI when no slug can be extracted."""
        mod, _, _, _, _ = _import_filesystem_handler()
        handler = _make_handler(mod)
        file_path = "/home/user/.gemini/chats/session-1.json"
        result = handler._get_gemini_branch(file_path, "gkey3")
        assert result == "GEMINI"

    def test_unresolved_slug_uses_uppercase(self):
        """Should use slug.upper() when resolution returns None."""
        mod, _, _, _, _ = _import_filesystem_handler()
        handler = _make_handler(mod)
        file_path = "/home/user/.gemini/tmp/my-project/chats/session-1.json"
        with patch.object(handler, "_resolve_gemini_slug", return_value=None):
            result = handler._get_gemini_branch(file_path, "gkey4")
...
class TestShortenModel:
    ...
    def test_gemini_model(self):
        """Should shorten gemini models, removing preview suffix."""
        mod, _, _, _, _ = _import_filesystem_handler()
        result = mod.MonitoringFileHandler._shorten_model("gemini-3-flash-preview")
...
# =============================================
# EXTRACT GEMINI ACTION TESTS
# =============================================


class TestExtractGeminiAction:
    """Tests for _extract_gemini_action static method."""

    def test_user_message(self):
        """User messages should return user message."""
        mod, _, _, _, _ = _import_filesystem_handler()
        cls = mod.MonitoringFileHandler
        result = cls._extract_gemini_action({"type": "user"})
        assert "User message" in result

    def test_non_gemini_type_returns_none(self):
        """Non-user, non-gemini types should return None."""
        mod, _, _, _, _ = _import_filesystem_handler()
        cls = mod.MonitoringFileHandler
        result = cls._extract_gemini_action({"type": "system"})
        assert result is None

    def test_gemini_with_tool_calls(self):
        """Gemini message with tool calls should format last tool."""
        mod, _, _, _, _ = _import_filesystem_handler()
        cls = mod.MonitoringFileHandler
        msg = {
            "type": "gemini",
            "toolCalls": [
                {"displayName": "first_tool"},
                {"displayName": "last_tool"},
            ],
        }
        result = cls._extract_gemini_action(msg)
        assert "last_tool" in result

    def test_gemini_with_tool_calls_name_fallback(self):
        """Gemini tool call should fall back to name if no displayName."""
        mod, _, _, _, _ = _import_filesystem_handler()
        cls = mod.MonitoringFileHandler
        msg = {"type": "gemini", "toolCalls": [{"name": "my_tool"}]}
        result = cls._extract_gemini_action(msg)
        assert "my_tool" in result

    def test_gemini_thinking(self):
        """Gemini message with thoughts should return Thinking."""
        mod, _, _, _, _ = _import_filesystem_handler()
        cls = mod.MonitoringFileHandler
        msg = {
            "type": "gemini",
            "thoughts": "some thinking",
            "content": [],
        }
        result = cls._extract_gemini_action(msg)
        assert "Thinking" in result

    def test_gemini_text_content(self):
        """Gemini message with text content should return text snippet."""
        mod, _, _, _, _ = _import_filesystem_handler()
        cls = mod.MonitoringFileHandler
        msg = {
            "type": "gemini",
...
# =============================================
# EXTRACT TEXT FROM CONTENT TESTS
# =============================================


class TestExtractTextFromContent:
    """Tests for _extract_text_from_content static method."""

    def test_string_content(self):
        """String content should return the text."""
        mod, _, _, _, _ = _import_filesystem_handler()
        cls = mod.MonitoringFileHandler
        result = cls._extract_text_from_content("Hello world")
        assert "Hello world" in result

    def test_list_content_with_text(self):
        """List content with text dict should return the text."""
        mod, _, _, _, _ = _import_filesystem_handler()
        cls = mod.MonitoringFileHandler
        result = cls._extract_text_from_content([{"text": "some text"}])
    ...
    def test_empty_string(self):
        ...
        mod, _, _, _, _ = _import_filesystem_handler()
        cls = mod.MonitoringFileHandler
        result = cls._extract_text_from_content("")
        assert "Agent response" in result
    ...
    def test_list_with_empty_text(self):
        """List with empty text dict should return default."""
        mod, _, _, _, _ = _import_filesystem_handler()
        cls = mod.MonitoringFileHandler
        result = cls._extract_text_from_content([{"text": "   "}])
        assert "Agent response" in result


# =============================================
# PARSE GEMINI ACTIVITY TESTS
# =============================================


class TestParseGeminiActivity:
    """Tests for _parse_gemini_activity."""

    def test_parses_new_messages(self):
        """Should parse new messages and emit agent event."""
        mod, _, _, _, queue = _import_filesystem_handler()
        handler = _make_handler(mod, queue=queue)
        ...
        with patch("builtins.open", _mopen(read_data=session_data)):
            result = handler._parse_gemini_activity("/fake/session.json", "GEMINI")
        assert result is True
        queue.enqueue.assert_called()

    def test_no_new_messages(self):
        """Should return True without emitting when no new messages."""
        mod, _, _, _, queue = _import_filesystem_handler()
        handler = _make_handler(mod, queue=queue)
        handler._jsonl_positions["/fake/session.json"] = 2
        session_data = json.dumps(
            {
                "messages": [
                    {"type": "user"},
    ...
    def test_deduplicates_action(self):
        """Should not emit duplicate actions."""
        mod, _, _, _, queue = _import_filesystem_handler()
        handler = _make_handler(mod, queue=queue)
        handler._last_agent_action["/fake/session.json"] = "User message"
        session_data = json.dumps({"messages": [{"type": "user"}]})
        with patch("builtins.open", _mopen(read_data=session_data)):
            result = handler._parse_gemini_activity("/fake/session.json", "GEMINI")
        assert result is True

    def test_extracts_model(self):
        """Should extract model from gemini messages."""
        mod, _, _, _, queue = _import_filesystem_handler()
        handler = _make_handler(mod, queue=queue)
        session_data = json.dumps(
            {
        ...
        with patch("builtins.open", _mopen(read_data=session_data)):
            handler._parse_gemini_activity("/fake/session.json", "GEMINI")
        assert "gemini" in handler._session_models.get("/fake/session.json", "")
    ...
    def test_json_error_returns_false(self):
        ...
        handler = _make_handler(mod, queue=queue)
        mock_path = MagicMock()
        mock_path.name = "session.json"
        with patch("builtins.open", _mopen(read_data="not json at all")):
            result = handler._parse_gemini_activity(mock_path, "GEMINI")
    ...
    def test_no_queue_still_returns_true(self):
        """Should return True even when no queue."""
        mod, _, _, _, _ = _import_filesystem_handler()
        handler = _make_handler(mod, queue=None)
        session_data = json.dumps(
            {
    ...
    def test_skips_none_actions(self):
        """Should skip messages that produce None actions."""
        mod, _, _, _, queue = _import_filesystem_handler()
        handler = _make_handler(mod, queue=queue)
        session_data = json.dumps({"messages": [{"type": "system"}]})
        with patch("builtins.open", _mopen(read_data=session_data)):
            result = handler._parse_gemini_activity("/fake/session.json", "GEMINI")
        assert result is True
        queue.enqueue.assert_not_called()
...
class TestHandleEvent:
    ...
    def test_gemini_json(self):
        """Should parse Gemini JSON session files."""
        mod, _, mock_bd, mock_filters, queue = _import_filesystem_handler()
        handler = _make_handler(mod, queue=queue)
        mock_filters.should_monitor.return_value = True
        mock_bd.detect_branch_from_path.return_value = "GEMINI"
        with patch.object(handler, "_get_gemini_branch", return_value="PRAX"):
            with patch.object(
                handler,
                "_parse_gemini_activity",
                return_value=True,
            ) as mock_parse:
                handler._handle_event(
                    "modified",
                    "/home/user/.gemini/tmp/slug/chats/session.json",
                )
                mock_parse.assert_called_once()
