...
Purpose: Capture the decision just made into compass (the rated decision engine) with Patrick's rating and note. Patrick fires this when he notices a decision worth recording — he supplies the judgement, you supply the decision text from the conversation. This is the human-triggered answer to the "noticing" problem: Patrick notices, you describe and store.
...
1. Parse `$ARGUMENTS`:
   - First token = `rating`. It MUST be one of `good | bad | impressive | interesting`. If it isn't, don't guess — ask Patrick which rating he meant and stop.
   - Everything after the first token = `note` (Patrick's observation; may be empty).
2. From the recent conversation, identify the decision being rated. Compose TWO short, concrete, single-line strings:
   ...
   This is your job: Patrick rated it, you describe it accurately from what just happened.
3. Store it (source is `patrick`, since he triggered the rating):
   ```
   drone @devpulse compass add "<context>" "<decision>" --rating <rating> --note "<note>" --source patrick
   ```
...
- If the decision Patrick means is ambiguous, ask before storing. One good entry beats a vague one.
